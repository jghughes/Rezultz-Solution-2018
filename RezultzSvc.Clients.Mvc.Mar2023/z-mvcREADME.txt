To debug these controllers, go to RezultzSvc.Clients.Mvc.Mar2023.ControllerEndpoints 
and comment in/out the mvc endpoint/s which point to local host. Ensure that you comment them out again for compiling for production!

Debugging a REST service is complicated. for a detailed readyreckoner, see ERROR LOG FOR REST API FROM NOVEMBER 2019.docx. 
To find this, open Word and search for "ERROR LOG FOR REST API FROM NOVEMBER 2019". It is stored on OneDrive for safety.
...
The routes for controllers are assigned in the ctor of the client for a controller, like so:-

	ThisControllerRoute = $"{ControllerEndpoints.AzureStorageControllerEndpoint}/{Routes.AzureStorageController}"

Which translates to:-

	Debug:		https://localhost:5196/Rezultz/AzureStorage
	Production: https://rezultzsvcmvc.azurewebsites.net/Rezultz/AzureStorage
...
When you click Start this project, three things occur, the console opens, the Swagger UI opens in your browser, and your test client project launches. 
...
properly, the Rest Api endpoints hard coded there are those displayed by Swagger. In my case I store the endpoints  
in RezultzSvc.Clients.Mvc.Mar2023.ControllerEndpoints. If they are correct, your test client will effortlessly connect to the API project 
and you can set breakpoints for a full debugging experience in both the API project and the test client. 
