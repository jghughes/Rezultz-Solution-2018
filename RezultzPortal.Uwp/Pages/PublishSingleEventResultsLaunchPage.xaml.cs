...
namespace RezultzPortal.Uwp.Pages
{
    public sealed partial class PublishSingleEventResultsLaunchPage
    {
        ...
        private async void BtnBrowseHardDriveForFileAndUpload_OnClick(object sender, RoutedEventArgs e)
        {
            const string failure = "Unable to import and upload file.";
            const string locus = "[BtnBrowseHardDriveForFileAndUpload_OnClick]";
            ...
            try
            {
                ...
                var datasetEntityName = JghFilePathValidator.AttemptToMakeValidNtfsFileOrFolderNameByReplacingInvalidCharacters('_', $"{DateTime.Now.ToString(JghDateTime.SortablePattern)}+{importFile.Name}");

                var uploadDidSucceed = await GlobalRaceResultsPublishingSvcAgent.UploadDatasetFileToBeProcessedSubsequentlyAsync(buttonVm.DatasetIdentifyingEnum,
        ...
        private async void BtnSavePreprocessedDataToHardDrive_OnClick(object sender, RoutedEventArgs e)
        {
            const string failure = "Unable to save file.";
            const string locus = "[BtnSavePreprocessedDataToHardDrive_OnClick]";

            var savedFileNameOfConvertedData = string.Empty;
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(PagesViewModel.SuccessfullyComputedResultsAsXml))
                {
                ...
                try
                {
                    ...
                    if (string.IsNullOrWhiteSpace(file?.Name))
                    {
                        ...
                        savedFileNameOfConvertedData = StringsPortal.Incomplete__nothing_saved;
                        PagesViewModel.SavedFileNameOfProcessedDataTextVm.Text = savedFileNameOfConvertedData;
                        return;
                    }
                    else
                    {
                        savedFileNameOfConvertedData = file.Name;
                        PagesViewModel.SavedFileNameOfProcessedDataTextVm.Text = savedFileNameOfConvertedData;
                    }
                }
                ...
                var contentToBeSaved = PagesViewModel.SuccessfullyComputedResultsAsXml;
            ...
            catch (Exception ex)
            {
                ...
                {
                    PagesViewModel.SavedFileNameOfProcessedDataTextVm.Text = StringsPortal.Incomplete__nothing_saved;
                }
