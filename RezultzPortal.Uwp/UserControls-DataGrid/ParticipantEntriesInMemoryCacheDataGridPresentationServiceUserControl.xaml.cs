...
namespace RezultzPortal.Uwp.UserControls_DataGrid
{
    ...
    ///     DataGridDesignerForItemsInRepository.GetNonEmptyColumnSpecificationItemsForRawParticipantHubItemEntries() to generate the
    ...
    ///     right here.
    ///     Step 3: create a presenter and call DataGridOfItemsInRepository.PopulatePresenterAsync() to fill the ItemsSource
    ///     property of the presenter.
    ///     If you look in RegisterParticipantsWorkingPage (that contains this control), you will see that
    ///     PortalParticipantAdminPagesVm.DataGridOfItemsInRepository is specified in XAML as the datacontext for the Telerik
    ///     datagrid.
    ///     This means that its ItemsSource property provides the row collection for the RadDataGrid.
    /// </summary>
    public sealed partial class ParticipantEntriesInMemoryCacheDataGridPresentationServiceUserControl : IParticipantEntriesInMemoryCacheDataGridPresentationService
    {
        private object _previouslyTappedItem;

        public ParticipantEntriesInMemoryCacheDataGridPresentationServiceUserControl()
        {
            InitializeComponent();
        }
