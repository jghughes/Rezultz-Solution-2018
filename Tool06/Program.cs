...
internal class Program
{
    ...
    [
        new Tuple<string, string>("Expert Results.txt", "Kelso2023mtb-results-01-expert.xml"),
        new Tuple<string, string>("Sport Results.txt", "Kelso2023mtb-results-01-sport.xml"),
        new Tuple<string, string>("Novice Results.txt", "Kelso2023mtb-results-01-novice.xml")
    ];
    ...
    private static async Task Main()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                {
                    //var pathOlFile = OutputFolderForXmlResults + @"\" + Path.GetFileNameWithoutExtension(fileItem.OutputFileName) + "." + StandardFileTypeSuffix.Xml;
            ...
            var outputOfProcessing =
                await RaceResultsPublishingSvcAgent.ProcessPreviouslyUploadedSourceDataIntoPublishableResultsForSingleEventAsync("21portal", "TEST MTB Series 2022", "Dummy event #13 August 16th", testSeriesProfile, importFileTargets);
            JghConsoleHelper.WriteLineFollowedByOne("answer:");
    ...
    {
        var outPutFileName = JghFilePathValidator.AttemptToMakeValidNtfsFileOrFolderNameByReplacingInvalidCharacters('-', DateTime.Now.ToString(JghDateTime.SortablePattern)) + "______ResultsSynthesisedByPublishingSvc" + "." +
